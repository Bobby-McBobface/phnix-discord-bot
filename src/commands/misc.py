...
async def villager(message: discord.Message, parameters: str, client: discord.Client) -> None:
    ...
    """
    last_was_alpha = False  # Was the last character part
    # of the alphabet? Used to detect the start of a word
    last_was_h = False  # Was the last character converted
    # to an H or h? Used to prevent 'H's without 'm's
    last_was_lower_m = False  # Was the last character
    # converted to a lowercase m? Used to make "HmmHmm"
    # instead of "HmmMmm"
...
@command({
    ...
    "role_requirements": {configuration.MODERATOR_ROLE},
    "category": Category.OTHER,
    "description": "Fades the colour of a role to an other \
                    colour (hex) in a certain time period (h, d or m)"
})
async def faderolecolour(message: discord.Message, parameters: str, client: discord.Client) -> None:
    global samples_scale
    ...
    if not util.is_hex(new_colour.lstrip('#')):
        await message.channel.send(
            content="Hm...:thinking: " + new_colour + " does not look like a colour in hex form!")
        return
    ...
    if samples is None:
        await message.channel.send(
            content="This does not look like a valid period... Try something like 3**d** for days, " \
    ...
    for i in range(0, samples):
        ...
        # If duration is non-positive, immediately
        # update colour to the requested one
