...
async def automod(message: discord.Message, client: discord.Client) -> bool:
    ...
    if await phishing_check(message):
        ...
        await logger.log_misc("Phishing check triggered",
...
async def filter_words(message: discord.Message) -> bool:
    ...
    for word in configuration.WORDS_CENSORED:
        # Make word lowercase to avoid always
        # false match on listed words with capitalized letters
...
async def phishing_check(message: discord.Message) -> bool:
    ...
    # Splitting the URL into parts, namely link eg.
    # "foobar.co.uk" and top level domain eg, "foobar"
    ...
    # Running a word similarity test on the top level
    # domain, comparing it to "discord". (Lower = more similar)
    # https://en.wikipedia.org/wiki/Damerau%E2%80%93Levenshtein_distance
    distance = jellyfish.damerau_levenshtein_distance(domain_parts[-2], 'discord')

    # If the distance is 0, the domain is "discord",
    # which isn't used by any illegitemate website
    if distance == 0:
        return False

    # Distances lower than 3 are highly likely to be phising.
    # This includes addresses like diacord.gift or dlisord.com
    if distance <= 3:
        await message.delete()
        return True

    # Distances lower than 5 are still likely to be phising,
    # but also have high false positive rates, so are sent
    # for secondary checking
    if distance <= 5:
        return await secondary_phising_check(message)

    # At distances over 5, it's unlikely that a link mascarades as "discord",
    # although composite links ("discord-nitro-gift.site") may go through
    # due to size
    return False


async def secondary_phising_check(message: discord.Message) -> bool:
    # The secondary check looks at the context of the message
    # to determine if it's suspicious enough to remove

    # Sleeping asynchronously to give time to discord to load
    # website embeds onto the message
    ...
    # Similarly, checking for more specific combinations
    # of keywords in the message body
    suspicious_message_words = ("@everyone", "free nitro", "discord nitro", "steam nitro")
