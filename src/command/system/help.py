...
class Help(Command):
    def __init__(self):
        super().__init__("help", "Shows a command info or the list of the commands", ["commands", "?"], [
            Parameter("command")], category=Category.SYSTEM)
        ...
        if not parameters:
            ...
            for _cmd in command_list:
                ...
                if last_category != cmd.category and last_category:
                    # We moved on to a new category, time to append the previous one to the new embed
                    embed.add_field(name=last_category.value["friendly_name"], value=category_commands, inline=False)
