...
async def log_moderation(message: discord.Message,
...
async def send_logs(message: str, client: discord.Client,
                    embed: Union[discord.Embed, None]) -> None:
    try:
        await client.get_guild(configuration.GUILD_ID)\
            .get_channel(configuration.LOG_CHANNEL).send(message,
                                                         embed=embed,
